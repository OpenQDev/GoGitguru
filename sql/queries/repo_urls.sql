...
WITH selected AS (
    ...
    UPDATE repo_urls_v2 SET status = 'queued'::repo_status, updated_at = NOW() WHERE url IN (SELECT url FROM selected)
    RETURNING *
)
